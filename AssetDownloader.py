...
def start_smart_compression(ogg_data_dict):
    ...
    with ProcessPoolExecutor(max_workers=workers) as executor:
        ...
        with tqdm(
            total=len(tasks), desc="Optimizing Assets", unit="file", dynamic_ncols=True
        ) as pbar:
            saved_count = 0
...
class AssetDownloader:
    ...
    def downloadAssets(self, assetIndexUrl: str, versionIndex: int):
        ...
        with open(
            f"dist/assets/indexes/{versionIndex}.json", "w", encoding="utf-8"
        ) as f:
            f.write(
                json.dumps(assetIndexData, separators=(",", ":"), ensure_ascii=False)
            )
        f.close()
        ...
        for name, obj in tqdm(objects.items(), desc="Updating asset index"):
            ...
            # Remove some bloat
            sname: str = name
            if sname.startswith("realms/"):
                continue
            if sname.startswith("minecraft/lang/"):
                continue
            ...
            downloadUrl = (
                f"https://resources.download.minecraft.net/{first_2}/{sha1_hash}"
            )
        ...
        for name, obj in tqdm(objects.items(), desc="Gathering audio files"):
            ...
            sname: str = name
            if sname.endswith(".ogg") == False:
                continue
            sha1_hash = obj["hash"]
            first_2 = sha1_hash[:2]
            asset_path = f"dist/assets/objects/{first_2}/{sha1_hash}"
            if (asset_path in toDownloadPaths) == False:
                continue
